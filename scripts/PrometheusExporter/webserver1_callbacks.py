...
def onHTTPRequest(webServerDAT, request, response):
	...
	response['data'] = "\n".join(f"{key}: {value}" for key, value in serialize_metrics(op('exportable')).items())
	return response

def serialize_metrics(data) -> dict[str, str]:
	resp: dict[str, str] = {}
	for i in range(data.numChans):
		resp[data[i].name] = data[i].eval()
	return resp
